...
// ── Muscle ID iz baze → SVG group ID ─────────────────────────────────────────
const MUSCLE_TO_SVG = {
  bicep_long:        ["biceps"],
  bicep_short:       ["biceps"],
  brachialis:        ["biceps"],
...
function buildColorMap(primary = [], secondary = []) {
  const map = {};
  secondary.forEach((id) => {
    (MUSCLE_TO_SVG[id] || []).forEach((svgId) => {
      if (map[svgId] !== "primary") map[svgId] = "secondary";
    });
  });
  primary.forEach((id) => {
    (MUSCLE_TO_SVG[id] || []).forEach((svgId) => {
...
function gStyle(svgId, colorMap) {
  const s = colorMap[svgId];
  if (s === "primary")   return { color: "#ef4444", opacity: 1 };
  if (s === "secondary") return { color: "#fb923c", opacity: 1 };
  return { color: "rgba(255,255,255,0.13)", opacity: 1 };
}

// ── PREDNJI SVG (Musclewiki) ──────────────────────────────────────────────────
function FrontSVG({ colorMap }) {
  const g = (id) => gStyle(id, colorMap);
...
// ── ZADNJI SVG (Musclewiki) ───────────────────────────────────────────────────
function BackSVG({ colorMap }) {
  const g = (id) => gStyle(id, colorMap);
...
function Legend() {
  return (
    <div style={{ display: "flex", gap: 14, justifyContent: "center", marginTop: 8 }}>
      {[["#ef4444", "Primarni"], ["#fb923c", "Sekundarni"], ["rgba(255,255,255,0.13)", "Neaktivni"]].map(([color, label]) => (
        <div key={label} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(255,255,255,0.5)" }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: color, border: color.includes("rgba") ? "1px solid rgba(255,255,255,0.2)" : "none" }} />
          {label}
        </div>
      ))}
    </div>
...
export default function MuscleDiagram({ primaryMuscles = [], secondaryMuscles = [], size = "small" }) {
  const colorMap = useMemo(
    () => buildColorMap(primaryMuscles, secondaryMuscles),
    [primaryMuscles, secondaryMuscles]
  );

  const hasMuscles = primaryMuscles.length > 0 || secondaryMuscles.length > 0;
  const maxH = size === "large" ? 420 : 300;

  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
      <div style={{ display: "flex", gap: 4, width: "100%", maxHeight: maxH }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <FrontSVG colorMap={colorMap} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <BackSVG colorMap={colorMap} />
        </div>
      </div>
      {hasMuscles && <Legend />}
    </div>
